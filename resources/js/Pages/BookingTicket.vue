                ...
                <v-col cols="12" md="8">
                    <div class="card" v-if="!showPaymentSection">
                        <div class="card-body">
                            <div v-for="row in seats" :key="row.label" class="seat-row">
                                ...
                                <div class="flex-grow flex justify-center">
                                    <Seat v-for="seat in row.seats" :key="seat.id" :seat-id="seat.id"
                                        :seat-number="seat.number" :seat-type="row.seatType?.name" :status="seat.status"
                            ...
                            <!-- Seat Type Legend -->
                            <div class="legend">
                                <div v-for="seatType in seatTypes" :key="seatType.name"
                                    class="flex flex-col items-center gap-1">
                                    <span class="text-xs text-zinc-200 font-normal">
                                        {{ seatType.name }}
                                    </span>
                                    <img :src="getLegendImage(seatType.name)" alt="Seat type" class="w-10 h-10" />
                    ...
                    <div class="card !text-zinc-300" v-if="showPaymentSection">
                        <div class="card-body p-4 space-y-5">
                            ...
                            <v-select v-model="form.payment_method_id" :items="paymentMethods" item-title="name"
                                item-value="id" label="Select Payment Method" prepend-inner-icon="mdi-credit-card"
                                variant="outlined" hide-details clearable solo class="text-zinc-200" />

                            <!-- Guest Email Field -->
                            <v-text-field v-model="form.guest_email" label="Guest Email (optional)" type="email"
                                prepend-inner-icon="mdi-email" variant="outlined" hide-details clearable solo
                                class="text-zinc-200" />

                            <!-- Submit Button -->
                            <div class="flex justify-end pt-4">
                                <v-btn :disabled="!paymentChosen" class="!font-semibold" variant="flat" color="#991f1f"
                                    @click="submitPayment">
                                    Submit Payment
                                </v-btn>
                            </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="my-4">
                                ...
                                <div class="flex flex-wrap gap-2 mt-3">
                                    <v-chip v-for="label in selectedSeatLabels" :key="label" color="white"
                                        class="px-3 !font-semibold" variant="tonal" size="small">
                                        {{ label }}
                                    </v-chip>
                                    <span v-if="selectedSeatLabels.length === 0"
                                        class="text-sm font-italic text-zinc-400">None</span>
                                </div>
                            </div>
                            ...
                            <div class="bg-white/10 rounded-3 px-5 py-4 flex justify-between items-center">
                                ...
                                <span class="text-xl font-bold text-white">$ {{ totalAmount }}</span>
                            </div>

                            <div class="flex justify-center mt-4">
                                <v-btn color="#991f1f" class="!font-semibold w-full"
                                    :variant="showPaymentSection ? 'outlined' : 'flat'"
                                    :class="selectedSeatIds.length > 0 ? '' : 'd-none'" @click="handleContinue">
...
<script setup>
    ...
    /**
     * Watch for changes in the selected seat IDs if the payment method is not selected, block the submission
     *
     * @returns {void}
     */
    watch(form, (newValue) => {
        if (newValue) {
            paymentChosen.value = !!newValue.payment_method_id;
        }
    }, {
        deep: true,
    });
