    ...
    <Modal v-slot="{ close }">
        <div class="form-container">
            <div class="form-header">
               <h2 class="form-title">{{__('Import Classification')}}</h2>
           ...
           <vee-form :validation-schema="schema" @submit="submitForm" >
             <div class="form-content">
                    ...
                    <v-file-input v-model="form.file" :error-messages="form.errors.file" accept=".xlsx"
                            :label="__('Please select Excel file')" prepend-icon="" prepend-inner-icon="mdi-file-excel" show-size
...
<script setup>
    ...
    const submitForm = (close) => {
        ...
        if (form.file) {
            form.post(route('dashboard.classifications.import'), {
                preserveState: true,
                preserveScroll: true,
                forceFormData: true,
                onSuccess: () => {
